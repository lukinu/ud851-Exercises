...
public class MainActivity extends AppCompatActivity {
    ...
    public void addToWaitlist(View view) {
        ...
        String personName = mNewGuestNameEditText.getText().toString();
        // COMPLETED (9) First thing, check if any of the EditTexts are empty, return if so
        if (personName.isEmpty() || partySizeString.isEmpty()) {
            return;
        }
    ...
    private void addGuest(String name, Integer size) {
        ...
        Long insertResult = mDb.insert(WaitlistContract.WaitlistEntry.TABLE_NAME, null, contentValues);
        Log.d(LOG_TAG, " Inserted a row " + insertResult);
    }
