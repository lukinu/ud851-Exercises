...
public class AddTaskActivity extends AppCompatActivity {
    ...
    public void onClickAddTask(View view) {
        ...
        EditText etTaskDescription = (EditText) findViewById(R.id.editTextTaskDescription);
        if (etTaskDescription.length() == 0) {
            return;
        }
        String taskDescription = etTaskDescription.getText().toString();
        ContentValues values = new ContentValues();
        values.put(TaskContract.TaskEntry.COLUMN_DESCRIPTION, taskDescription);
        values.put(TaskContract.TaskEntry.COLUMN_PRIORITY, mPriority);

        // COMPLETED (7) Insert new task data via a ContentResolver
        Uri resultUri = getContentResolver().insert(TaskContract.TaskEntry.CONTENT_URI, values);
